...
function parseLatexToOmml(expr: string): MathComponent[] {
  ...
  while (remaining.length > 0) {
    ...
    const subSupMatch = remaining.match(/^([^\\^_{}]+)(?:\^\{([^{}]*)\}_\{([^{}]*)\}|_\{([^{}]*)\}\^\{([^{}]*)\})/);
    if (subSupMatch) {
...
export function renderMathExpression(expr: string): string {
  const replacements: [RegExp, string][] = [
    ...
    [/\\subset/g, '⊂'], [/\\supset/g, '⊃'], [/\\in/g, '∈'], [/\\notin/g, '∉'],
    // Arrows
    [/\\leftarrow/g, '←'], [/\\rightarrow/g, '→'], [/\\Rightarrow/g, '⇒'],
    ...
    [/_8/g, '₈'], [/_9/g, '₉'], [/_n/g, 'ₙ'],
    // Fractions
    [/\\frac\{1\}\{2\}/g, '½'], [/\\frac\{1\}\{3\}/g, '⅓'],
    [/\\frac\{1\}\{4\}/g, '¼'], [/\\frac\{3\}\{4\}/g, '¾'],
  ];
